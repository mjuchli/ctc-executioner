...
A sample period of the collected data set is then being investigated in order to find and visualize properties of the market situation and behaviour of the market participants.
...
Therefore, as a last section of this chapter, we construct features which cover the stated hypotheses.
...
Being aware that the following observations are taken from a random sample of a historical data set, the intention is to make a suggestion which properties might be worth to consider in the feature construction process.
...
We therefore calculate the mean price and accumulate the sizes of the trades with equal order side over a fixed amount of time (in seconds) $\Delta{s}$.
That is,
\begin{equation}
    Trade' = (ts', os', \frac{\sum_{i=ts}^{ts+\Delta{s}}p_i}{ts+\Delta{s}}, \sum_{i=ts}^{ts+\Delta{s}}q_i)
