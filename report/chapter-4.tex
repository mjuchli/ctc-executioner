...
The volume has been investigated as a sum of events over time.
In order to understand the behaviour of the participants in greater detail, a \textit{volume map} will provide better insight into the single events occurred over time, as shown in Figure \ref{fig:data-volmap-crated}.
The x-axis represents the time stamp and the y-axis is the volume of the placed order.
...
Most of the orders were placed with volume between >0 and <0.5 and significantly less orders had a larger volume.
Only a few orders had a volume greater than 10 BTC, meaning that most of the participants are either willing to buy and sell only small quantities, or split their orders to minimize the market impact.
...
This might be evidence of someone splitting one larger order into small pieces (known as buy or sell wall) and is marked with an orange-dashed circle.
Surprisingly, this behaviour appeared the most immediately before and during the market price started rising again (compare time stamps from Figure \ref{fig:data-price-movement}).
Last but not least, examples of areas dominated by one particular order side (buy or sell) is marked with a purple box, indicating that for a certain period of time either buyers or sellers flooded the market with orders with various volume.
...
Continuous cancellations become especially evident at volume levels just below $10^0$ and $10^{-1}$ and correlate with the continuous orders created at the same price level, as shown above.
...
A cancellation of one of the created buy-walls is particularly evident at the same time stamp and with equal volumes as previously discovered during the observation of created orders.
That makes it likely that the wall was created and cancelled by a single trader.
\vfill
\pagebreak
...
Not all of those limit orders might have resulted into a trade and therefore
...
Furthermore, intensive and immediate purchases are visible with various volume at time stamps before and during the price fall.
We remember that there was one spike towards a higher price during the dip, and the time stamp of which correlates strongly with the purchases visible in this figure.
\\
\\
...
Patterns were found and a hypothesis was stated which encourages the various offerings of volume of an order to be an indicator of future behaviour of market participants, which would eventually influence the market price and allows determines optimal order placement.
...
The volume of these trades are illustrated as bubbles with a size that determines traded volume and the position defines the price.
...
Before and during the dip sells appear consecutively, followed by a series of buy orders with low volume, which caused the minor spike.
Interestingly, a rather large trade caused by a bid appeared shortly before the price started to rise again.
