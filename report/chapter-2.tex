...
Reinforcement Learning is a specific learning approach in the Machine Learning field and aims to solve problems which involve \textit{sequential decision making}.
...
\begin{figure}[H]
    \centering
    \makebox[\linewidth]{
        \includegraphics[width=\linewidth]{rl-overview.png}
    }
...
\\
\\
With the interfaces provided above \ref{rl-mdp}, we can define an interaction process between an agent and environment by assuming discrete time steps: $t=0, 1, 2, ...$
...
Value iteration algorithms converge to the \textit{optimal action-value} function $Q_{i} \rightarrow Q^*$ as $i \rightarrow \infty$. \cite{sutton1998reinforcement}

\subsubsection{Q-Learning}

One of the reinforcement learning algorithms used in this work is known as Q-Learning. 
The name derives from the application of the previously presented Q-function (Eq. \ref{eq:q-function}), more specifically its use within the Bellman equation (\ref{eq:bellman}) which is iteratively updated.
\\
\\
The general idea is ... 
...
The term \textit{generalization} refers to the action-value function (Eq. \ref{eq:optimal-action-value-function}) and the fact that this value is estimated for each state separately, which becomes totally impractical in for large state spaces that oftentimes occur in real world scenarios.
