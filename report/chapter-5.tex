...
\For{episode$=1,M$}
    \State Initialize sequence $s_1={x_1}$ with observation and preprocessed sequenced $\phi{}_1=\phi{(s_1)}$
    \For{t=1,T}
        \State With probability $\epsilon$ select a random action $a_t$
        \State otherwise select $a_t=\max{}_aQ^{*}(\phi{(s_t)}, a; \theta)$
        \State Execute action $a_t$ in emulator and observe reward $r_t$ and observation state $x_{t+1}$
